...
class DetailPageTestCase(TestCase):
    ...
    def setUp(self):
        ...
        new_product = Product.objects.create(
            name="New Product", nutriscore=5, category_id=new_category_id)

    # Test of return HTTP 200
    def test_detail_page_returns_200(self):
        new_product_id = Product.objects.get(name="New Product").id
        response = self.client.get(
            reverse('catalogue:detail', args=(new_product_id,)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_proposition_for_substitution(self):
        new_product_id = Product.objects.get(name="New Product").id
        old_context = self.client.get(
            reverse('catalogue:detail', args=(new_product_id,))).context

        new_category_id = Product.objects.get(name="New Product").id
        new_product_2 = Product.objects.create(
            name="New Product 2", nutriscore=1, category_id=new_category_id)

        new_product_id = Product.objects.get(name="New Product").id
        new_context = self.client.get(
            reverse('catalogue:detail', args=(new_product_id,))).context
